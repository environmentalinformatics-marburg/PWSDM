...
# #unzip temperature data - only useful for first extraction
...
#assign WGS84 (EPSG: 4326) to gridded world population rasters (see .txt file in
#directory)
for(i in seq(gpw)){
  proj4string(gpw[[i]]) <- CRS("+init=epsg:4326")
}

#assign WGS84 (EPSG: 4326) to Hyde population data
for(i in seq(popc)){
...
newex <- extent(c(-180,180,-58,85)) #extent matching all data sets
...
#write stacks
saveRDS(tempstack, file=paste0(main, "run/temp.rds"))
saveRDS(popstack, file=paste0(main, "run/pop.rds"))
